...
import { useFollowUser, useGetUserById } from "@/lib/react-query/queries";
...
const Profile = () => {
  ...
  const handleFollowUser = (
    ...
    if (followersArray.includes(user.id)) {
      followersArray = followersArray.filter((Id) => Id !== user.id);
        ...
        <div className="flex xl:flex-row flex-col max-xl:items-center flex-1 gap-7">
          ...
          <div className="flex justify-center gap-4">
            ...
            <div className={`${user.id === id && "hidden"}`}>
              {checkIsFollowed(following, currentUser.$id) ? (
                <Button
                  type="button"
                  className="shad-button_primary px-8"
                  onClick={(e) => handleFollowUser(e, currentUser.$id)}
                >
                  UnFollow
                </Button>
              ) : (
        ...
        <div className="flex max-w-5xl w-full">
          <Link
            ...
            className={`profile-tab rounded-l-lg ${
              pathname === `/profile/${id}` && "!bg-light-3 dark:!bg-dark-3"
            }`}
          >
            <img
